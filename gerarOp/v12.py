...
##############################################
    ...
    ## codigosfora = ['FIL-MASSA-424','FIL-MASSA-446','FIL-MASSA-713', 'FIL-MASSA-770', 'FIL-MASSA-770-90' , 'FIL-MASSA-789', 'MIX-MASSA-536']
    ...
    def iniVetPerda(codigo, revEst):
        ...
        sg1SQL += "LEFT JOIN ( "
        sg1SQL += "SELECT DISTINCT G1_COD, G1_COMP, B1_UM "
        sg1SQL += "FROM SG1010 SG1 "
    ...
    def SetorColetor(codigo):
        selectSetor ="select RTRIM(B1_ZZLNPRD) from SB1010 SB1 (NOLOCK) "
        where = " WHERE UPPER(B1_COD) = '" +codigo.strip(" ")+ "' AND D_E_L_E_T_ = ''"
        
        cursor.execute(selectSetor + where)
        vetColRastro = cursor.fetchall()

        local = vetColRastro[0][0]
        print(local + "|")
        if local == 'WS-TPV' or local == 'WS-GR' or local == 'WS-MIX': 
            sControl = True
        else:
            sControl = False
    ...
    def inicializaKanban(kanban, PartNumber, verificaKanban, maquina):
        ...
        if(verificaKanban):
            ...
            setor = str(tabAp['CO'+str(k)].value).upper()
    ...
    for k in range(12,tabAp.max_row+1):
        ...
        if(str(tabAp['CI'+str(k)].value) != "None"):
            setorDest = ''
            coletaCod = ''
            kanban = ''
            setorOrigem = ''
            ...
            qtdMovimentacao = float(tabAp['CI'+str(k)].value)

            if(str(tabAp['CG'+str(k)].value) != "None"):
                coletaCod = str(tabAp['CG'+str(k)].value).upper()
            else:
            ...
            tipoMovimentacao = str(tabAp['CK'+str(k)].value)
            ...
            if(str(tabAp['CM'+str(k)].value) != "None"):
                coletaDest = str(tabAp['CM'+str(k)].value).upper()
            ...
            if(indSenbetsu):
                ...
                coletaOrigem = str(tabAp['CO'+str(k)].value).upper()     
    ...
    for kanban in resumoAp.keys():
        ...
        if kanban == 'O-261':
            for maquina in resumoAp[kanban].keys():  
                ...
                sControl = SetorColetor(resumoAp_ref["PN"])

                if not lControl and not sControl:
    ...
    if(len(listMovimentacoes) > 0):
        ...
        for k in range(0,len(listMovimentacoes)):
            ...
            sControl = SetorColetor(listMovimentacoes[k][1])
            ...
            if(not lControl and listMovimentacoes[k][3] == "BAIXA"):
