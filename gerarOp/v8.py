...
##############################################
                ...
                def inicializaKanban(kanban, PartNumber):
                    ...
                    if(str(tabAp['E'+str(k)].value) != "None"): 
                        ...
                        if(str(tabAp['AF'+str(k)].value) != "None" and tabAp['AF'+str(k)].value > 0): 
                            ...
                            if(resumoAp[kanban]["Setor"] != "WS-GR"):
    ...
    arq = open(pathNew+'\\02 - PERDA '+nOP+'.txt', 'w')
    ...
    for kanban in resumoAp.keys():
        ...
        lControl = verificaRastro(resumoAp[kanban]["PN"])
        
        if not lControl:
            if(resumoAp[kanban]["totProd"] > 0):          
                linhaOP = str(nOP+";"+resumoAp[kanban]["Item"]+";001;"+resumoAp[kanban]["PN"] +";" + str(round(resumoAp[kanban]["totProd"],2))+";"+data +";"+data+";F\n")
                x.write(linhaOP)
